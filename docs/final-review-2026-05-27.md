# Revisión final esqueleto Beauly — 2026-05-27
...
4. **`generateMetadata` solo en 3 rutas indexables.** `src/app/[locale]/page.tsx` (landing) y `src/app/[locale]/buscar/page.tsx` (búsqueda) no exportan `generateMetadata` propio. Heredan del layout (`title.template = '%s | Beauly'`) que sí está bien configurado, pero entonces ninguna de las dos rutas indexables aporta un `<title>` específico. Tras los SEO mocks de G2, conviene cerrar esta brecha antes de abrir el sitio a la cliente. Esfuerzo: 30 min cada una.
